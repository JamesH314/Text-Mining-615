...
# write.table(my_book,'Book.txt',row.names = F)
...
```{r echo=T}
q26 <- tnum.query('hssb/book1# has text',max=60)
df26 <- tnum.objectsToDf(q26)
df26 %>% select(subject:string.value)%>% head()
```

```{r}
# tnBooksFromLines(my_book$text, "hssb/book1")
...
```{r}
df27<- tnum.query('hssb/book1/section# has text',max=7000) %>% tnum.objectsToDf()
#df27 %>% view()
